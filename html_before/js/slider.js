...
Swiper.prototype = {
    init: function () {
        ...
        for (let i = 0; i < this.retImgArr.length; i++) {
            li += `
            <li style="left: ${i * this.moveWidth}px;
            width: ${this.moveWidth}px; 
            background-image: url(${this.retImgArr[i].imgPath});
            background-size: cover; background-repeat: no-repeat;"
            class="swiper-item">
            <a href="${this.retImgArr[i].url}"></a>
            </li>
            `
            ;
        }
        ...
        for (let i = 0; i < this.imgArr.length; i++) {
            if (i === 0) {
                spotLi += `<li class="spot-item" style="background-color: #07beff;" index=${i}></li>`;
            } else {
    ...
    initDom() {
        ...
        setInterval(() => {
            if (this.nowIndex===0) {
                ...
                swiperitem1 = document.querySelector('.swiper-item1'),
                swiperitem2 = document.querySelector('.swiper-item2'),
                swiperselect = document.querySelector('.swiper-select')
                swiperitem1.classList.remove('none')
                swiperitem2.classList.add('none')
                swiperselect.classList.add('none')
                topbar.style.backgroundColor = '#5c686d'
                ...
                swiperitem2 = document.querySelector('.swiper-item2'),
                swiperselect = document.querySelector('.swiper-select')
                swiperitem1.classList.add('none')
                swiperitem2.classList.remove('none')
                swiperselect.classList.remove('none')
    ...
    setActiveSpot: function () {
        for (let i = 0; i < this.swiperSpotDom.childElementCount; i++) {
            if (i === Math.abs(this.nowIndex)) {
                document.getElementsByClassName('spot-item')[i].style.backgroundColor = '#07beff'
            } else {
    ...
    eventBind() {
        ...
        this.swiperSpotDom.addEventListener('click', function (e) {
            ...
            if (target.tagName.toLowerCase() === "li") {
                ...
                that.nowIndex = index;
                that.setActiveSpot();
                that.mainDom.style.transition = `left .8`
